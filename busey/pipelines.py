...
class BuseyPipeline:
    ...
    # Database setup
    conn = sqlite3.connect('busey.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `busey`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM busey WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO busey (date, title, link, content)"
